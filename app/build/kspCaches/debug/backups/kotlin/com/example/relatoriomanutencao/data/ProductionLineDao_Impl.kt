...
public class ProductionLineDao_Impl(
  ...
  private val __updateAdapterOfProductionLine: EntityDeleteOrUpdateAdapter<ProductionLine>
  init {
    this.__db = __db
    ...
    this.__updateAdapterOfProductionLine = object : EntityDeleteOrUpdateAdapter<ProductionLine>() {
      protected override fun createQuery(): String = "UPDATE OR ABORT `production_lines` SET `id` = ?,`name` = ? WHERE `id` = ?"
  ...
  public override suspend fun updateProductionLine(productionLine: ProductionLine): Unit = performSuspending(__db, false, true) { _connection ->
    __updateAdapterOfProductionLine.handle(_connection, productionLine)
  }
